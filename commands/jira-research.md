...
Deep investigation of a JIRA issue to find existing work and related context across all forks.
...
- **Extract key terms** for deep search:
  ...
  - Component names (e.g., "transformer", "predictor", "storage initializer", "routes", "authconfig")
  - Behavioral keywords (e.g., "nil pointer", "timeout", "race condition")

### Step 2: Determine Target Repo

Use `workspace.mdc` and `repo-scope.mdc` to determine which repo this JIRA targets (kserve or odh-model-controller). This affects which forks to search.

### Step 3: Deep Search for Existing Work

**Search GitHub** across all forks using GitHub MCP:
...
- `opendatahub-io/odh-model-controller` (ODH)
- `red-hat-data-services/odh-model-controller` (downstream)
...
### Step 5: Check for Existing Investigation Comment
...
### Step 6: Create/Update JIRA Comment
...
*Target repo:* {{kserve or odh-model-controller}}
...
> Fix exists in [source]. Run `/jira-work {{jira_key}} target:odh/release-v0.17` to cherry-pick.
...
2. Target repo determined
3. Key terms extracted for search
4. Deep search across all forks (GitHub + Slack)
5. Findings compiled and presented to user
6. **Prompt:** "Post investigation notes to JIRA? [y/n/edit]"
7. If yes, create or update comment
8. Recommendation displayed with next steps
